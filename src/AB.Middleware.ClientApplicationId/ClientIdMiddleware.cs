...
{
    /// <summary>
    /// Middleware which attempts to parse a Client Application Id.  Application Id's are unique string identifiers used to identify
    /// logical applications (a.k.a Clients).
    /// </summary>
    public class ClientIdMiddleware
    {
        ...
        {
            StringValues clientId = SetClientId(context);
        ...
        private StringValues SetClientId(HttpContext context)
        {
            ...
            if (clientIdFoundInRequestHeader == false) clientId = string.Empty;
            return clientId;
        }
    }
}
